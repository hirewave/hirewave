...
public class ClientService {
    ...
    public void incrementCompleted(String clientId) {
        ClientEntity existing = getEntityById(clientId);
        ClientEntity updated = new ClientEntity(
                existing.id(), existing.name(), existing.email(),
                existing.completedProjects() + 1, existing.cancelledProjects()
        );
    ...
    public void incrementCancelled(String clientId) {
        ...
        ClientEntity updated = new ClientEntity(
                existing.id(), existing.name(), existing.email(),
                existing.completedProjects(), existing.cancelledProjects() + 1
        );
        clientRepository.save(updated);
    }

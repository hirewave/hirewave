...
public class FreelancerService {
    ...
    public FreelancerResponse updateFreelancer(String id, UpdateFreelancerRequest request) {
        ...
        FreelancerEntity updated = new FreelancerEntity(
                ...
                request.skills(), request.hourlyRate(),
                existing.averageRating(), existing.totalRatings(), existing.ratingSum()
        );
    ...
    public FreelancerResponse addRating(String freelancerId, int rating) {
        FreelancerEntity existing = getEntityById(freelancerId);
        int newRatingSum = existing.ratingSum() + rating;
        int newTotalRatings = existing.totalRatings() + 1;
        double newAverage = (double) newRatingSum / newTotalRatings;
        FreelancerEntity updated = new FreelancerEntity(
                existing.id(), existing.name(), existing.email(),
                existing.skills(), existing.hourlyRate(),
                newAverage, newTotalRatings, newRatingSum
        );
        return toResponse(freelancerRepository.save(updated));
    }
    ...
    private FreelancerResponse toResponse(FreelancerEntity entity) {
        return new FreelancerResponse(
                ...
                entity.averageRating(), entity.totalRatings()
        );
    }
}
